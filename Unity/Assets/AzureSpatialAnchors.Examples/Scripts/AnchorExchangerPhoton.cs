...
{
    public class AnchorExchangerPhoton
    {
        private string _anchorKey = "anchorKey";
        ...
        {
            return await RetrieveLastAnchorKey();
        }

        public async Task<string> RetrieveLastAnchorKey()
        {
            await Task.Delay(100);
            return (string)PhotonNetwork.CurrentRoom.CustomProperties[_anchorKey];
        }

        internal async Task<long> StoreAnchorKey(string anchorKey)
        {
            PhotonRoom.AddCustomProperty(_anchorKey, anchorKey);
            await Task.Delay(100);
            return 1;
